...
@push('styles')
    ...
    .excluirForumButton{
        margin-top: 15px;
        background-color: #ff4949c9 !important;
        box-shadow: 2px 2px 1px 0.2rem #d43e3e3b !important;
    }
...
@section('content')
    ...
    <div class = "paiMain">
        ...
        <div class="tituloConteudo">Edição de Fórum</div>

        <!-- ================== Form de edicao ===================== -->

        <form method="post" action="{{route('forum.editarForumGravar')}}" enctype="multipart/form-data">
            @csrf
                <div class="form-group">
                    <label for="nomeforum"><img class = "iconLogin" src="{{asset('Imagens/email.svg')}}"/>Nome do Fórum</label>
                    <input value="{{$forumAtual->nome}}" type="text" name='nome' class="form-control" id="nomeforum" aria-describedby="emailHelp" placeholder="Exemplo: Meu Fórum sobre gatos">
                </div>

                <div class="form-group">
                    <label for="detalhesForum"><img class = "iconLogin" src="{{asset('Imagens/page.svg')}}"/>Detalhes do Fórum</label>
                    <input value="{{$forumAtual->descricao}}" type="text" class="form-control" name='descricao' id="detalhesForum" aria-describedby="detalhesForum" placeholder="Exemplo: Esse é um fórum divertido sobre gatos">
                </div>

                <input type="hidden" name="id_forum" value="{{$forumAtual->id}}"/>
                ...
                <button type="submit" class="btn btn-primary botaoLogin">Salvar Alterações</button>
                <a href="{{route('forum.exclurForum', $forum)}}" class="btn btn-primary botaoLogin excluirForumButton">Excluir Fórum</a>
            </form>
        </div>
        </div>
    </div>
